...
const homeSpeakerInit = () => {
  ...
  if (window.innerWidth < 990) {
    ...
    const contentHeight = slide.querySelector(".slide__content").offsetHeight;
...
const agenda = () => {
  ...
  var daysHeight = agenda.querySelector(".agenda__days").getBoundingClientRect()
    .height;
    ...
    inputs.forEach(input => {
      input.addEventListener("change", () => {
        ...
        setTimeout(() => {
          ...
          theHeight = (agendaHeight - daysHeight) * -1;
        }, 400);
      });
    });
  };
...
const fixDays = () => {
  ...
  const agenda = document.querySelector("#agenda");
  const sweetSpot =
    document.querySelector(".agenda__header").getBoundingClientRect().height +
...
function hamburger() {
  ...
  const links = document.querySelectorAll(".nav-link:not(.nav-link--overview)");
      ...
      scrollTop() {
        return new Promise(resolve => {
          ...
          tl.to(obj, 0.8, {
            ...
            ease: Power4.easeOut
          }).to(container, 0.8, { autoAlpha: 0, ease: Power4.easeOut }, 0.1);
        });
      },
...
window.addEventListener("load", function() {
  tippy(".home-sponsors__sponsor", {
    ...
    content: function(reference) {
      return reference.querySelector(".tippy-template");
    }
  });
});

document.body.addEventListener("click", function(event) {
  if (event.target.classList.contains("tippy-close")) {
    event.preventDefault();
